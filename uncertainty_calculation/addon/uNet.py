...
def unet_block(mcd_pos, drop_rate, n_depth=2, n_filter_base=16, kernel_size=(3,3), n_conv_per_depth=2,
               activation="relu",
               batch_norm=False,
               dropout=0.0,
    ...
    def _func(input):
        ...
        for n in range(n_depth):
            ...
            if mcd_pos == 'Down' or mcd_pos == 'Full':
                layer = Dropout(drop_rate)(layer, training = True)
        ...
        if mcd_pos == 'Mid' or mcd_pos == 'Full':
            layer = Dropout(drop_rate)(layer, training = True)
